...
export default function Directory() {
  ...
  const handlePrintProfile = () => {
    ...
    const printHTML = generatePrintableHTML(profile);
  ...
  const generatePrintableHTML = (profile) => {
    const createRoleHTML = (role) => {
      const status = (profile[role.key] || 'N').toUpperCase();
      const serviceNum = profile[`${role.key} Service`] || '';
      const qtyNum = profile[`${role.key.replace(/ /g, '_')}_Service_Qty`] || '';
      
      return `
        <div class="role-item">
          <div class="role-name">${role.name}</div>
          <div class="role-status-cell"><span class="role-status status-${status}">${status}</span></div>
          <div class="role-last-cell"><span class="service-number">${serviceNum}</span></div>
          <div class="role-qty-cell"><span class="quantity-number">${qtyNum}</span></div>
        </div>
      `;
    };
    
    const teamRolesHTML = ROLE_CONFIG.team.map(createRoleHTML).join('');
    const profRolesHTML = ROLE_CONFIG.professor.map(role => {
      const status = (profile[role.key] || 'N').toUpperCase();
      const serviceNum = profile[`${role.key} Service`] || '';
      const qtyNum = profile[`${role.key}_Service_Qty`] || '';
    ...
    const isDeceased = profile.Deceased === true || (profile.Deceased || '').toString().toLowerCase() === 'y';
    const isDoNotCall = profile.Do_Not_Call === true || (profile.Do_Not_Call || '').toString().toLowerCase() === 'y';
    let statusClass = '';
    if (isDeceased) statusClass = ' deceased';
    else if (isDoNotCall) statusClass = ' do-not-call';
    ...
    return `
      ...
      <head>
        ...
        <style>
          ...
          .profile-main-info { background-color: #f8f9fa; padding: 15px; border-radius: 8px; margin-bottom: 15px; border: 1px solid #dee2e6; page-break-inside: avoid; }
          ...
          .roles-section { margin-top: 15px; page-break-inside: avoid; }
          ...
          .team-grid { grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(14, auto); }
          ...
          .status-N { background-color: #dc3545 !important; }
          .status-I { background-color: #ffc107 !important; color: #333 !important; }
          .status-E { background-color: #28a745 !important; }
          ...
          .bar-segment.pass { background-color: #28a745 !important; }
          .bar-segment.fail { background-color: #dc3545 !important; }
          .profile-main-info.do-not-call { border-left: 30px solid #dc3545 !important; background-color: rgba(220, 53, 69, 0.05) !important; position: relative; }
          ...
          .profile-main-info.deceased { border-left: 30px solid #000000 !important; background-color: rgba(0, 0, 0, 0.05) !important; position: relative; }
      ...
      <body>
        <div class="page-container">
          <div class="profile-main-info${statusClass}">
          ...
          <div class="roles-section">
            <div class="roles-title">Rector Qualification</div>
          ...
          <div class="legend">
            <div class="legend-item"><div class="legend-color status-N"></div><span>N - Never</span></div>
          ...
          <div class="roles-section">
            ...
            <div class="role-header-legend team-headers">
              <div class="role-header-set"><div class="role-header-name">Role</div><div class="role-header-status">Status</div><div class="role-header-last">Last</div><div class="role-header-qty">Qty</div></div>
              <div class="role-header-set"><div class="role-header-name">Role</div><div class="role-header-status">Status</div><div class="role-header-last">Last</div><div class="role-header-qty">Qty</div></div>
              <div class="role-header-set"><div class="role-header-name">Role</div><div class="role-header-status">Status</div><div class="role-header-last">Last</div><div class="role-header-qty">Qty</div></div>
            </div>
          ...
          <div class="roles-section">
            ...
            <div class="role-grid professor-grid">${profRolesHTML}</div>
          </div>
        </div>
      </body>
      </html>
